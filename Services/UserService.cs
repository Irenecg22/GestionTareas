...
public class UserService : IRestService<Usuario>
{
    ...
    public async Task<bool> LoginAsync(string email, string password)
    {
        try
        {
            ...
            {
                Debug.WriteLine($"?? Token vacío o nulo en la respuesta");
                return false;
            }
            Debug.WriteLine($"??? Token recibido: {tokenResponse.AccessToken.Substring(0, 20)}...");
    ...
    public async Task<Usuario?> GetCurrentUserAsync()
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(token))
            {
                Debug.WriteLine($"?? GetCurrentUserAsync: No hay token disponible");
                return null;
            }
            _client.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue("Bearer", token);
            ...
            {
                Debug.WriteLine($"?? Token expirado o inválido, limpiando token de SecureStorage");
                Logout();
            }
            return null;
        }
    ...
    public async Task<(bool Success, int StatusCode, string? Message)> UpdateUserAsync(int userId, UsuarioUpdateRequest request)
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(token))
            {
                Debug.WriteLine($"?? UpdateUserAsync: No hay token disponible");
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                Debug.WriteLine($"?? ===== FIN ACTUALIZACIÓN =====");
            ...
            Debug.WriteLine($"?? ===== FIN ACTUALIZACIÓN (ERROR) =====");
